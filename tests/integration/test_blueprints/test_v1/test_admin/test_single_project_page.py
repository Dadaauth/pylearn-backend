"""
Test cases for /api/v1/admin/project/<project_id> endpoint
"""


def test_get_single_project_page_success(app, client, admin, auth, create_projects):
    with app.test_request_context():
        ...
        response = client.get(f"/api/v1/admin/project/{projects[1]["id"]}", headers={
...
def test_get_single_project_page_user_not_logged_in(app, client, create_projects):
    with app.test_request_context():
        ...
        response = client.get(f"/api/v1/admin/project/{projects[1]["id"]}")
        data = response.json

        assert response.status_code == 401
        assert data.get('data') is None

def test_get_single_project_page_user_role_not_admin(app, client, student, auth, create_projects):
    with app.test_request_context():
        app.preprocess_request()
        course, module, projects = create_projects
...
def test_get_single_project_page_invalid_project_id(app, client, admin, auth):
    with app.test_request_context():
        app.preprocess_request()

        auth_r = auth.login(admin.username, "test_password", "admin")
        response = client.get(f"/api/v1/admin/project/invalid-project-id", headers={
            "Authorization": f"Bearer {auth_r.json['data']['access_token']}"
        })
        data = response.json
...
def test_get_single_project_page_no_next_and_prev_project(app, client, admin, auth, create_project):
    with app.test_request_context():
        app.preprocess_request()
        course, module, project = create_project

        auth_r = auth.login(admin.username, "test_password", "admin")
        response = client.get(f"/api/v1/admin/project/{project["id"]}", headers={
            "Authorization": f"Bearer {auth_r.json['data']['access_token']}"
        })
        data = response.json
