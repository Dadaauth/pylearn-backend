"""
Test cases for /api/v1/auth/is_logged_in endpoint
"""
import os
from datetime import date, timedelta


def test_admin_user_role_success(client):
    ...
    response = client.get('/api/v1/auth/user/role', headers={
...
def test_mentor_user_role_success(app, client):
...
def test_student_user_role_success(app, client):
...
def test_user_role_no_authentication(client):
    response = client.get('/api/v1/auth/user/role')
    assert response.status_code == 401

def test_user_role_wrong_method(client):
    response = client.post('/api/v1/auth/user/role')
